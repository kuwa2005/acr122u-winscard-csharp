...
ターゲットフレームワークはいずれも `.NET Framework 4.6.2` です。SDK-style project ではなく、Visual Studio 2017 世代の旧形式 `.csproj` です。
...
- この環境では `dotnet` SDK 10.0.204 は利用できました。
- `msbuild` は PATH 上では見つかりませんでした。
- `dotnet build .\NFC_CardReader.sln --no-restore` は、`.NETFramework,Version=v4.6.2` の参照アセンブリ不足により失敗しました。ビルドには .NET Framework 4.6.2 Developer Pack / Targeting Pack の導入が必要です。
