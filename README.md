...
2026-05-25 時点では、新規 console project `Acr122uCardInspector` は未実装です。既存 `CardReader_TestConsole` には Phase 1 の一部であるリーダー概要、firmware、ATR / UID / ATS 表示、トレース、重複表示抑制が追加されていますが、[Phase 1 検証レポート](docs/testing/new-card-info-viewer-phase1-test-report.md) では未完了、[品質チェックレポート](docs/testing/new-card-info-viewer-quality-report.md) では要修正、[コードレビュー](docs/reviews/new-card-info-viewer-code-review.md) では差し戻し判定になっています。
