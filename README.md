...
新しいカード情報表示プログラムは `Acr122uCardInspector` です。通常起動では ACR122U 候補リーダーを自動選択し、カードの安定検出後に ATR / UID / ATS / ATR 由来の推定カード名を表示します。既定動作は read-only で、reader 設定変更、書き込み系 APDU、鍵探索、FeliCa サービス総当たり、残高/履歴/個人情報の読み取りは行いません。
